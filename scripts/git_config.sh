...
alias pushup           '!args=""; [ "x$1" = "x-n" ] && args="-n" && shift; b="${2:-`git current-branch`}"; if [ "$b" = "`git main-branch`" ]; then echo "no pushup on main"; exit 1; fi; git push $args -u "${1:-`git pushup-target`}" HEAD:"`git branch-name "$b"`"'
